...
<?php
    ...
    $connection = new mysqli('localhost', 'root', '', 'hopital1');
    ...
    function login($email_id_unsafe, $password_unsafe, $table = 'users')
    {
        ...
        if ($num_rows > 1) {
            ...
            echo "<div class='alert alert-success'> <strong>Super!</strong> Vous êtes connectés</div>";
    ...
    {
        $success = "<div class='alert alert-success'> <strong>Réussi!</strong>";
        $fail = "<div class='alert alert-warning'><strong>Désolé!</strong>";
        $end = '</div>';

        switch ($type) {
            case 'record-success':
                return "$success Enregistrement réussi avec succés! $end";
                break;
            case 'record-fail':
                return "$fail Echec de l'enregistrement. $end";
                break;
            case 'record-dup':
                return "$fail Enregistrement dupliqué. $end";
                break;
            case 'no-match':
                return "$fail il semble que vos identifiants sont pas correctes, veuillez réessayer. $end";
                break;
            case 'con-failed':
                return "$fail Echec de la Connection! $end";
                break;
            case 'appointment-success':
                return "$success Votre Rendez-Vous a été sauvegardé avec succés! Votre N° de RV est de : $data $end";
                break;
            case 'appointment-fail':
                return "$fail Désolé votre Rendez-Vous n'a pas été sauvegardé! $end";
                break;
            case 'update-success':
                return "$success Nouveau enregistrement Mise à jour avec succés! $end";
                break;
            case 'update-fail':
                return "$fail Echec de la Mise à Jour des données! $end";
                break;
            default:
                // code...
                break;
    ...
    function enter_patient_info($date_rdv_unsafe,$heure_rdv_unsafe,$full_name_unsafe, $age_unsafe, $weight_unsafe, $phone_no_unsafe, $address_unsafe)
    {
        global $connection, $error_flag,$result;
  
        $full_name = ucfirst(secure($full_name_unsafe));
        $date_rdv = secure ($date_rdv_unsafe );
        $heure_rdv = secure ($heure_rdv_unsafe);
        $age = secure($age_unsafe);
        $weight = secure($weight_unsafe);
        $phone_no = secure($phone_no_unsafe);
        $address = secure($address_unsafe);
  
        $sql = "INSERT INTO `patient_info` VALUES (NULL,'$date_rdv','$heure_rdv', '$full_name', $age,$weight, '$phone_no','$address');";
      ...
      function getPatientsFor($doctor = 'Dentiste')
      {
          global $connection;
      ...
      function getAllAppointments()
      {
          ...
          return $connection->query('SELECT appointment_no, full_name,date_rdv,heure_rdv,speciality, medical_condition FROM patient_info, appointments where patient_info.patient_id = appointments.patient_id');
      }
      ...
      function getAllPatientDetail($appointment_no)
      {
          ...
          return $connection->query("SELECT appointment_no, full_name,date_rdv,heure_rdv, dob, weight, phone_no, address, medical_condition FROM patient_info, appointments where appointment_no=$appointment_no AND patient_info.patient_id = appointments.patient_id;");
      }
